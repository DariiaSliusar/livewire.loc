...
{{--                @foreach($limitList as $k => $v)--}}
{{--                    <option @if($v == $limit) selected @endif wire:key="{{ $k }}" value="{{ $v }}">--}}
{{--                        {{ $v }}--}}
{{--                    </option>--}}
{{--                @endforeach--}}
{{--            </select>--}}
    ...
    <div class="table-responsive position-relative">
        <div wire:loading style="position: absolute; width: 100%; height: 100%; background: rgba(255, 255, 255, 0.7); text-align: center; padding-top: 20px;">
        ...
        <table class="table table-striped table-hover">
            <thead>
            <tr>
                <th wire:click="changeOrder('users.id')" style="cursor:pointer;"><x-sort-arrows fieldName="ID"
                                                                        :orderByField="$orderByField"
                                                                        :orderByDirection="$orderByDirection"
                                                                        :orderByFieldList="$orderByFieldList"/>
                </th>
                <th wire:click="changeOrder('users.name')" style="cursor:pointer;"><x-sort-arrows fieldName="Name"
                                                                          :orderByField="$orderByField"
                                                                          :orderByDirection="$orderByDirection"
                                                                          :orderByFieldList="$orderByFieldList"/></th>
                <th wire:click="changeOrder('users.email')" style="cursor:pointer;"><x-sort-arrows fieldName="Email"
                                                                           :orderByField="$orderByField"
                                                                           :orderByDirection="$orderByDirection"
                                                                           :orderByFieldList="$orderByFieldList"/></th>
                <th wire:click="changeOrder('countries.name')" style="cursor:pointer;"><x-sort-arrows fieldName="Country"
                                                                              :orderByField="$orderByField"
                                                                              :orderByDirection="$orderByDirection"
                                                                              :orderByFieldList="$orderByFieldList"/></th>
                <th>Delete</th>
